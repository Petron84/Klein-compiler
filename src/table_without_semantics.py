try:
    def parsetable():
        ...
        rulemap = {
            ...
            "4": "def := function identifier ( parmlist ) : type body",
            "5": "parmlist := ε",
            "6": "parmlist := FP",
            "7": "FP := idtype FP'",
            "8": "FP' := ε",
            "9": "FP' := , idtype FP'",
            "10": "idtype := identifier : type",
            "11": "type := integer",
            "12": "type := boolean",
            "13": "body := printexp body",
            "14": "body := exp",
            "15": "printexp := print ( exp )",
            "16": "exp := simexp exp'",
            "17": "exp' := ε",
            "18": "exp' := = simexp exp'",
            "19": "exp' := < simexp exp'",
            ...
            "21": "simexp' := ε",
            "22": "simexp' := or term simexp'",
            "23": "simexp' := + term simexp'",
            "24": "simexp' := - term simexp'",
            "25": "term := factor term'",
            "26": "term' := ε",
            "27": "term' := * factor term'",
            "28": "term' := / factor term'",
            "29": "term' := and factor term'",
            "30": "factor := literal",
            "31": "factor := not factor",
            "32": "factor := - factor",
            "33": "factor := identifier factor'",
            "34": "factor := if simexp exp' then simexp exp' else simexp exp'",
        ...
        parsemap = {"PROGRAM": {"function": 1,"$":1},
                    ...
                    "EXP'": {"function": 17, "then": 17, "else": 17, "and": 17, "or": 17, ")": 17, "+": 17, "-": 17, "*": 17, "/": 17, "=": 18, ',': 17, "<": 19, "$": 17},
    ...
    print("Klein Error: Parse table failed to generate")
